...
const settingUpdaters: {
  ...
  // Phase 1/2/3 post-processing settings. Using (commands as any) so these work
  // both before and after bindings.ts is regenerated by `bun run tauri dev`.
  remove_fillers_enabled: (value) =>
    (commands as any).changeRemoveFillerEnabledSetting?.(value as boolean) ??
    (commands as any).changeRemoveFiltersEnabledSetting?.(value as boolean),
  ...
  fragmented_word_repair_enabled: (value) =>
    (commands as any).changeFragmentedWordRepairEnabledSetting?.(value as boolean),
